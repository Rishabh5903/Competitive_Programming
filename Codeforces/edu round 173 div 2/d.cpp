...
ll k1=(l+g-1)/g,k2=r/g;
pair<ll,ll> ans={INF+1,INF+1};ll maxo=0;
FOR(i,k1,min(k1+7,k2)+1){
    for(ll j=k2;j>=max(k2-7,k1);j--){
        if(gcd(i,j)==1){
        if((j-i)>=maxo){
            if((j-i)==maxo && ((i*g)>=ans.first))continue;
            maxo=j-i;ans={i*g,j*g};
        }}
    }
}
if(ans.first==(INF+1)){ce("-1 -1");}
