...
string s;
cin>>s;
for(ll i=0;i< n;i++){FOR(j,0,m)cin>>mat[i][j];
}
vl col(m,0),row(n,0);
FOR(i,0,n)FOR(j,0,m)row[i]+=mat[i][j];
FOR(j,0,m)FOR(i,0,n)col[j]+=mat[i][j];
ll r=0,c=0;
// ce(row);ce(col);
FOR(i,0,s.size()){
    if(s[i]=='D'){
        mat[r][c]=-row[r];col[c]-=row[r];row[r]=0;r++;
    }
    else{
        mat[r][c]=-col[c];row[r]-=col[c];col[c]=0;c++;
    }
    // coutv({r,c,mat[r][c]});
