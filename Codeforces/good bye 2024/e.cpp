...
vvl adj;vl par;ll x,n,ans;
void dfs(int i,int p=-1){
    par[i]=p;ll temp=0;
    for(ll j:adj[i]){
        if(adj[j].size()==1)temp++;
        if(j==p)continue;dfs(j,i);
    }
    ans+=x*max(((ll)adj[i].size()-temp-1)*(temp>0),0LL);
    // cout<<i<<" "<<ans<<" "<<adj[i]<<endl;
}
void solve() {
cin>>n;
adj=vvl(n);par = vl(n);
...
ll leaves=0;set<ll> s;
FOR(i,0,n)if(adj[i].size()==1){
    leaves++;s.insert(adj[i][0]);
}
x=(n-leaves-s.size());
ans=(n-leaves)*leaves;
dfs(0);
