...
unordered_map<char,ll> m2;
    ...
    if(temp.size()!=n){ce("NO");continue;}
    ll ans=1;
    FOR(i,0,n){
        if(m1.find(l[i])!=m1.end() && m1[l[i]]!=temp[i]){ans=0;ce("NO");break;}
        if(m2.find(temp[i])!=m2.end() && m2[temp[i]]!=l[i]){ans=0;ce("NO");break;}
        m1[l[i]]=temp[i];m2[temp[i]]=l[i];
