...
string s;
cin>>s;
vl v=primeFactorization(n);
sort(v.begin(), v.end()); v.erase(unique(v.begin(), v.end()), v.end());ll ans=0;
for(ll len:v){ll num=0;
    for(int i=0;i<(v.size()/len);i++){
        FOR(j,0,len){
            if(s[j]!=s[i*len+j])num++;
        }
    }if(num<=1)ans=maxm({ans, len});
}ce(ans);
