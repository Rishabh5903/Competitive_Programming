...
ll help(ll x,ll k){
    ...
    if(x<11 || k<2)return mp[{x,k}]=0;
    
    ll n=k;ll ans=0;
    if(n>(floor(log10(x))+1))return mp[{x,k}]=0;
    for(int i=n;i>=2;i--){
        string temp(i,'1');
        if(x%stoi(temp) == 0){
            // cout<<x<<" "<<temp<<endl;
            ans=1;break;}
        ans=ans|| help(x - stoi(temp),i) || help(x, i-1) || help(x - stoi(temp),i-1) ;
        if(ans)break;
    }
    return mp[{x,k}]=ans;
}
