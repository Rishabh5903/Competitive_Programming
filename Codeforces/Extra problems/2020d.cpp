...
ll n,m,a,d,k;
cin>>n>>m;
set<vl,Compare> s;ll ans=n;vvl queries(n,vl(3));
for(ll i=0;i< n;i++){cin>>queries[i][0]>>queries[i][1]>>queries[i][2];
}sort(all(queries));
for(auto& q:queries){
    if(!s.size()){s.insert({q[0],q[1],q[2]});ans-=q[2]+1;}
    else{
        a=(*s.rbegin())[0];
        d=(*s.rbegin())[1];
        k=(*s.rbegin())[2];
        if(!((q[0]-a)%d==0 && (q[0]-a)/d<=k)){}
        ll dd=lcm(d,q[1]);
