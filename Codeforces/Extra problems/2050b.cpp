...
ll sum1=0,sum2=0;
FOR(i,0,n){
    if(i%2)sum1+=l[i];else sum2+=l[i];
}
if(n%2){
    if(sum2%(n/2+1)==0 && (sum2/(n/2+1) == sum1/(n/2)) && sum1%(n/2)==0)ce("YES");else ce("NO");
}
else{
    if(sum2%(n/2)==0 && (sum2/(n/2) == sum1/(n/2)) && sum1%(n/2)==0)ce("YES");else ce("NO");
}
}
int main() {
