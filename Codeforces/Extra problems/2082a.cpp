...
vl cntr(n,0),cntc(m,0);
FOR(i,0,n)FOR(j,0,m)if(v[i][j]=='1')cntr[i]++;
FOR(i,0,m)FOR(j,0,n)if(v[j][i]=='1')cntc[i]++;
ll c1=0,c2=0;for(int i:cntr)if(i%2)c1++;for(int i:cntc)if(i%2)c2++;
ce(max(c1,c2));
}
int main() {
