...
ll l[n-1],pre[n-1];
for(ll i=0;i< n-1;i++){
cin>>l[i];if(!i)pre[i]=l[i];else pre[i]=pre[i-1]^l[i];
}
vl ans(n);
ll maxo=0,ind=-1;
FOR(i,0,n-1)if(pre[i]>maxo){maxo=pre[i];ind=i;}
ce(ind);
ans[ind+1]=n-1;
coutv(ans);
FOR(i,ind+2,n)ans[i]=ans[i-1]^l[i-1];
for(int i=ind;i>=0;i--)ans[i]=l[i]^ans[i+1];
couta(pre,n-1);
coutv(ans);
}
int main() {
