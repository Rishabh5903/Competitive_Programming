...
ll ans=0,prev1=0,prev2=0;
FOR(i,1,n){
    if(l[i]==1){if(l[i-1]!=1){ce(-1);return;}else continue;}
    ll temp=l[i];
    while(temp<l[i-1]){temp*=temp;prev2++;}
    while(prev1 && ((l[i-1]*l[i-1])<=l[i])){l[i-1]*=l[i-1];prev1--;}
    
    prev2+=prev1;ans+=prev2;
    // coutv({prev1,prev2,ans,temp,l[i],l[i-1]});
    prev1=prev2;prev2=0;
}
ce(ans);
}
