...
for(int i=0;i<v.size();i++){
    if(f)break;
    for(ll j:s)if(v[i].find(j)!=v[i].end()){v[i].insert(all(s));f=1;break;}
}
if(!f)v.pb(s);
}
vl take(v.size(),1);
...
FOR(i,0,v.size()){
    FOR(j,i+1,v.size()){
        if(!take[i])break;
        for(ll k:v[i])if(v[j].find(k)!=v[j].end()){take[i]=0;v[j].insert(all(v[i]));break;}
    }
    if(v[i].size())f=1;
}
ll ans=accumulate(all(take),0LL);
ce(ans-f);
}
int main() {
