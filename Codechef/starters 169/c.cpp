...
bool compare(const vector<ll>& a, const vector<ll>& b) {
    // Compare by the minimum value first
    ll min_a = min(a[0], a[1]);
    ll min_b = min(b[0], b[1]);
    
    if (min_a < min_b) return true;
    if (min_a > min_b) return false;
    
    // If the minimums are equal, compare by the maximum value
    ll max_a = max(a[0], a[1]);
    ll max_b = max(b[0], b[1]);
    
    return max_a <= max_b;  // Ensure correct comparison
}
...
void solve() {
    ...
    // Reading input as vectors of size 2
    for (ll i = 0; i < n; i++) {
        ll a, b;
        cin >> a >> b;
        v.push_back({a, b});  // Store each pair as a vector
    }
