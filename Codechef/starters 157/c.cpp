...
int expo(int a, int n, int md) {
    int res = 1; 
    while (n) {
        if (n & 1) { res = modmul(res, a, md); --n; } 
        else { a = modmul(a, a, md); n >>= 1; }
    }
    return res;
}
...
void solve() {
    ...
    while (true) {
        ...
        ans.pb({1, 3});
        vector<ll> curr(vec.begin(), vec.begin() + 3);
        vector<ll> curr2 = curr;
        sort(all(curr2));

        ll median = curr2[1];
        ll pos = find(all(curr), median) - curr.begin();
